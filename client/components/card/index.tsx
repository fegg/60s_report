...
import useFrontProjectList from '@/hooks/useFrontProjectList';
...
function Card(props: any) {
  ...
  if (type === 'item') {
    ...
    // 初始化一次事项弹窗
    const data = useItemList({ projectType, deptCode });
    works = data.works;
    worksMap = data.worksMap;
  } else {
    currentData = item[`${type}s`];

    // 初始化一次工作弹窗
    const data = useFrontProjectList({ timeScope });
  ...
  return (
    <CardContext.Provider
      value={{ getProjects, projects, projectsMap, works, worksMap, searchParams }}
    >
      <Block
        label={title}
      ...
      >
        {!!currentData.length &&
          currentData.map((data: any, index: number) => (
            <CardLayout
              type={type}
              key={`${type}_${index}_${data['projectId']}_${data.time}_${item[`${type}Type`]}`}
              data={data}
